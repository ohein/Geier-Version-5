...
public class IntelligentererGeier extends HolsDerGeierSpieler {
	...
	public int gibKarte(int naechsteKarte) {
		...
		if (naechsteKarte > 0) {
			// Kommt die hoechste Karte und der gener hat die gleiche höchste =>
			// spiele mittlere Karte
			if ((nochZuGewinnen.indexOf(naechsteKarte) == 0)
					&& (vomGegnerNochNichtGelegt.get(0) == nochNichtGespielt.get(0))) {
				ret = nochNichtGespielt.get(nochZuGewinnen.size() / 2);
				// System.out.println(">>> A");
				// Steht die zu gewinnende Karte in der vorderen Haelfte der
				// Reihenfolge und hat der gegner => Spiele die höchste Karte
